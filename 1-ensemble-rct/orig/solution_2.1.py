...
# ------------------------------------------------------------------------------
#
def get_stage_2b(sandbox, pname):
    '''
    Compute intermediate results.  Another `post_exec` callback will, after all
    tasks are done, evaluate the intermediate data and decide how to continue:
    ...
    t2 = re.Task()
    t2.executable = '/bin/sh'
    t2.arguments  = ['-c', 'tail -qn 1 power.*.txt | paste -sd+ | bc']
    t2.stdout     = 'result.txt'
    t2.sandbox    = sandbox
    t2.download_output_data = ['%s > %s.result.txt' % (t2.stdout, pname)]
    s2.add_tasks(t2)
    ...
    def post_exec(stage, pname):
        ...
        result = int(open('%s.result.txt' % pname).read().split()[-1])
        ...
        if result > LIMIT:
            ...
            pipeline.add_stages(get_stage_2b(sandbox, pname))
...
def generate_pipeline(pname):
    ...
    s2b = get_stage_2b(sandbox, pname)
